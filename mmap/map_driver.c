...
{  
   ...
   int i = 1;
   // 设备的注册    设备的主设备号，设备名称，所链接的file_operations结构
   ...
   {   // 对申请到的页框的标记位进行置位 告诉系统 这些页框已经被我们使用了
...
// 对mmap系统调用创建的vm_area_struct结构中的虚拟地址建立其相应的页表项
// 建立页表项有两种方法:
// 1.remap_pfn_range： 在执行此函数的时候直接给vm_area_struct每个页表项创建物理页表项（连续）
// 2.fault：进程访问到引起缺页的时候进行分配
static int mapdrv_mmap(struct file *file, struct vm_area_struct *vma)  
{  
    unsigned long offset = vma->vm_pgoff << PAGE_SHIFT;  
    unsigned long size = vma->vm_end - vma->vm_start;  
   
    // 如果进程中用来映射的线性地址区大于申请的物理地址
    if (size > MAPLEN) {  
        printk("size too big\n");  
        return -ENXIO;  
    }  
    /*  only support shared mappings. */  
    // 如果vm区权限是可写入的。vm_flags保存进程对该vm的访问权限的
    // 且在可写入的时候也应该是可共享的
    ...
    // 锁住这些物理页框，让他们不要被操作系统交换出去
    vma->vm_flags |= VM_LOCKONFAULT;  
    if (offset == 0) {  
        // 最重要的一步 也就是当前进程的vm缺页方法替换成了我们在模块中写的缺页处理程序
...
/* page fault handler */ 
// 把内核中的线性地址所对应的物理地址映射到用户空间的某个线性地址
int map_fault(struct vm_fault *vmf)  
{  
	struct page *page;
	void *page_ptr;
        unsigned long offset, virt_start, pfn_start;	
        // 产生缺页的线性地址在vma映射区中的偏移量
        offset = vmf->address - vmf->vma->vm_start;
        // 内核中的线性地址 线性区的起始地址加上每一页的偏移 就得到了内核中每一页的线性地址
        virt_start = (unsigned long)vmalloc_area + (unsigned long)(vmf->pgoff << PAGE_SHIFT);
        // vmalloc_to_pfn找到页帧 pfn_start为线性地址对应的物理地址的页框号
